...
### precautions. The table based on medians is for the main manuscript.
### A separate table shows means, which could be used in the supplement.
### Category 4 is excluded.
...
duration_iso_post_iso_table_path <- file.path(
  ...
  "table_manuscript_iso_post_iso_duration_3_cat_median.xlsx"
)

mean_duration_iso_post_iso_table_path <- file.path(
  results_dir,
  products_config$products$directory,
  "table_supplemental_iso_post_iso_duration_3_cat_mean.xlsx"
)
...
compare_guidance_df <- compare_guidance_df |> filter(symp_type_cat != 4)
...
openxlsx::addStyle(
  ...
  rows = c(2, 7, 12), cols = 3
)

openxlsx::addStyle(
  wb = duration_iso_wb, sheet = "Sheet1",
  style = openxlsx::createStyle(border = c("bottom")),
  rows = c(1, 6, 11), cols = 1:5, gridExpand = TRUE
)

openxlsx::addStyle(
  wb = duration_iso_wb, sheet = "Sheet1",
  style = openxlsx::createStyle(textDecoration = "bold"),
  rows = c(1, 2, 7, 12), cols = 1:5,
