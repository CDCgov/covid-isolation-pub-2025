...
main_symp_cat_fig_path_pdf <- file.path(
  results_dir,
  products_config$products$directory,
  "figure_manuscript_results_diff_averted_3_cat.pdf"
)

main_symp_cat_fig_path_png <- file.path(
  results_dir,
  products_config$products$directory,
...
supp_symp_cat_si_time_fig_path <- file.path(
  results_dir,
  products_config$products$directory,
  "figure_supplemental_results_diff_averted_3_cat_si_time.png"
)

diff_averted_summary_table_path <- file.path(
  results_dir,
  products_config$products$directory,
  "table_supplemental_results_diff_averted_3_cat.xlsx"
)
...
# and, also, further stratified by time to symptom improvement

mean_diff_by_symp_cat <- main_symp_cat_df |>
  dplyr::group_by(symp_type_cat) |>
  dplyr::summarise(mean_diff_averted = mean(diff_averted,
    na.rm = TRUE
  ))
...
mean_diff_by_si_time <- main_symp_cat_df |>
  dplyr::group_by(symp_type_cat, si_time) |>
  dplyr::summarise(mean_diff_averted = mean(diff_averted, na.rm = TRUE))

mean_diff_by_si_time_wt_I_t <- main_symp_cat_df |>
  dplyr::group_by(symp_type_cat, si_time) |>
  dplyr::summarise(wt_mean_diff_averted = weighted.mean(
    x = diff_averted,
    w = intrinsic_I_t, na.rm = TRUE
...
# by symptom categories 1-3, and overall (i.e., excluding category 4)
...
main_symp_cat_fig_fx <- function() {
  ...
  main_symp_cat_boxplot_list <- boxplot(
    ...
    at = 3:0, whisklty = 0, staplelty = 0,
    horizontal = TRUE, outline = F, col = NA, xaxt = "n", yaxt = "n",
    ylab = NA, xlab = NA, add = T, boxwex = 0.3
  )
  points(
    x = mean_diff_by_symp_cat_wt_I_t$wt_mean_diff_averted, y = 3:0,
    pch = 17
  )
  ...
  par(xpd = NA)
  arrows(x0 = -20, x1 = -40, y0 = -1, length = 0.1)
  arrows(x0 = 20, x1 = 40, y0 = -1, length = 0.1)
  ...
  hist_list <- vector("list", 4)
  max_counts <- vector("integer", 4)
  ...
  for (cat in 1:4) {
    hist_list[[cat]] <- hist(
      ...
      breaks = seq(from = -54.5, to = 54.5, by = 1), plot = FALSE
    )
  ...
  for (cat in 1:4) {
    ...
    y_coord_bottom <- (4 - cat) + dot_plt_width / 2
    y_coord_top <- (4 - cat) + 1 - dot_plt_width / 2
  ...
  return(main_symp_cat_boxplot_list)
}

pdf(
  file = main_symp_cat_fig_path_pdf,
  width = 6.5, height = 7
)
...
png(
  filename = main_symp_cat_fig_path_png,
...
# differences in transmission potential averted
# by symptom categories 1-3, and overall (i.e., excluding category 4)
# and stratified by symptom improvement time
# points are colored by antigen detection probability
# plot includes histograms above each dot plot
# plotted means are means weighted by total infectiousness
...
png(
  filename = supp_symp_cat_si_time_fig_path,
  width = 6.5, height = 8, units = "in", res = 300
)

par(oma = c(6, 4, 1, 3), xpd = F, yaxs = "r")
...
# loop through the symptom categories

par(mar = c(1, 1, 0, 0))

for (cat in 1:4) {
  cat_level <- levels(main_symp_cat_df$symp_type_cat)[cat]
  single_cat_df <- main_symp_cat_df |> dplyr::filter(symp_type_cat == cat_level)
  single_cat_mean_diff_by_si_time <- mean_diff_by_si_time_wt_I_t |>
    dplyr::filter(symp_type_cat == cat_level)
  single_cat_mean_diff <- mean_diff_by_symp_cat_wt_I_t |>
    dplyr::filter(symp_type_cat == cat_level)
  plot(
    x = single_cat_df$diff_averted,
    y = single_cat_df$si_time, xlim = c(x_min, x_max), ylim = c(y_min, y_max),
    pch = 16,
    col = grey_blue[single_cat_df$ant_det_col_level],
    xaxt = "n", yaxt = "n", ann = FALSE
  )
  points(
    x = single_cat_mean_diff_by_si_time$wt_mean_diff_averted,
    y = single_cat_mean_diff_by_si_time$si_time,
    pch = 17, cex = 0.6
  )
  ...
  if (cat == 4) {
    ...
    axis(side = 1, at = c(-40, -20, 0, 20, 40), labels = NA)
  }

  axis(side = 2, at = c(1, 10, 20, 28))
  text(
    x = x_min, y = 24,
    labels = as.character(levels(main_symp_cat_df$symp_type_cat)[cat]), pos = 4
  )
  boxplot(
    x = single_cat_df$diff_averted, at = -2, whisklty = 0, staplelty = 0,
    horizontal = TRUE, outline = F,
    col = "grey", xaxt = "n",
    ylab = NA, xlab = NA, ylim = c(x_min, x_max), add = T, boxwex = 5
  )
  points(x = single_cat_mean_diff$wt_mean_diff_averted, y = -2, pch = 17)
}

mtext(
  text = "Simulated days to observed symptom improvement",
  side = 2, cex = 0.7, lin = 2, outer = TRUE
)
...
par(xpd = NA)
arrows(x0 = -20, x1 = -40, y0 = -21, length = 0.1)
arrows(x0 = 20, x1 = 40, y0 = -21, length = 0.1)
...
# due to undefined difference in proportion averted and ignoring category 4
...
openxlsx::saveWorkbook(
  ...
  file = diff_averted_summary_table_path,
  overwrite = TRUE
)
